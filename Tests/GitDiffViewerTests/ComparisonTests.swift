...
struct ComparisonTests {
    ...
    func `statuses and exact renames come from blob ids`() {
        let comparison = Comparison(
            ...
            right: [entry("a.swift", "1"), entry("b.swift", "9"), entry("new/name.swift", "3"), entry("added.swift", "4")],
            leftSource: nil, rightSource: nil
        )

        #expect(comparison.statuses == ["a.swift": .same, "b.swift": .different, "old/name.swift": .renamed, "added.swift": .onlyRight])
    ...
    func `git renames refine the comparison without overriding exact ones`() {
        var comparison = Comparison(left: [entry("a.swift", "1"), entry("x.swift", "5")], right: [entry("b.swift", "2"), entry("y.swift", "5")], leftSource: nil, rightSource: nil)
    ...
    func `explorer trees carry directory statuses and honour the filter and style`() {
        ...
        let full = ExplorerTrees.build(comparison: comparison, leftTree: leftTree, rightTree: leftTree, showsChangesOnly: false, style: .hierarchy)
        #expect(full.statuses["a"] == .different)
        #expect(full.statuses["z"] == .same)
        #expect(full.left.map(\.id) == ["a", "z"])

        let filtered = ExplorerTrees.build(comparison: comparison, leftTree: leftTree, rightTree: leftTree, showsChangesOnly: true, style: .flat)
