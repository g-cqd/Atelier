...
struct SourceLoaderTests {
    ...
    func `a folder scan lists supported files by relative path and hashes only those small enough`() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: "gdv-folder-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: root.appending(path: "Sources/Nested"), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: root.appending(path: "node_modules"), withIntermediateDirectories: true)
        ...
        let entries = try await SourceLoader().entries(of: .directory(root)).sorted { $0.relativePath < $1.relativePath }
