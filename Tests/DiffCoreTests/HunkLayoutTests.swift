...
struct HunkLayoutTests {
    ...
    func `hunks are clipped to the document`() {
        #expect(HunkLayout.hunks(changeRanges: [0..<2, 98..<100], rowCount: 100, context: 3) == [0..<5, 95..<100])
    }
    ...
    func `expanding a gap reveals rows below the previous hunk and above the next one`() {
        let hunks = HunkLayout.hunks(
            ...
            rowCount: 100,
            context: 2,
            expansions: [1: GapExpansion(below: 5, above: 4), 0: GapExpansion(below: 0, above: 3), 2: GapExpansion(below: 10, above: 0)]
        )
    ...
    func `an expansion that closes a gap merges the hunks`() {
        let hunks = HunkLayout.hunks(changeRanges: [10..<11, 20..<21], rowCount: 100, context: 2, expansions: [1: GapExpansion(below: 100, above: 0)])
        #expect(hunks == [8..<23])
    }
    ...
    func `gap keys follow the base hunks even after an expansion merges two of them`() {
        ...
        let merged = HunkLayout.layout(changeRanges: ranges, rowCount: 60, context: 1, expansions: [1: GapExpansion(below: 100)])
        #expect(merged.hunks.map(\.rows) == [4..<14, 29..<32])
        #expect(merged.hunks.map(\.firstBase) == [0, 2])
        #expect(merged.baseCount == 3)

        let trailing = HunkLayout.layout(changeRanges: ranges, rowCount: 60, context: 1, expansions: [1: GapExpansion(below: 100), 3: GapExpansion(below: 10)])
