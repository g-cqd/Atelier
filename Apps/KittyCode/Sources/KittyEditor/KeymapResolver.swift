...
public struct KeymapResolver: Sendable {
    ...
    public init(config: KittyConfig) {
    ...
    public func resolve(_ stroke: KeyStroke, context: KeyContext, isRepeat: Bool = false) -> CommandID? {
    ...
    public func resolveSequence(_ strokes: [KeyStroke], context: KeyContext) -> ResolveResult {
    ...
    public func openedStatusHints() -> String {
        let save = shortcutLabel(for: .saveFile) ?? "^O"
        let quit = shortcutLabel(for: .handleCtrlX) ?? "^X"
