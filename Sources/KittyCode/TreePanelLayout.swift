...
enum TreePanelLayout {
    static func verticalScrollMetrics(
        rowCount: Int,
        scrollOffset: Int,
        in rect: Rect
    ) -> ScrollMetrics {
    ...
    static func contentWidth(rowCount: Int, in rect: Rect) -> Int {
        max(0, rect.width - (showsVerticalScrollIndicator(rowCount: rowCount, in: rect) ? 1 : 0))
    }

    static func verticalScrollIndicatorRect(rowCount: Int, in rect: Rect) -> Rect? {
