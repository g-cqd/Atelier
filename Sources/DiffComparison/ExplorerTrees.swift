...
package struct ExplorerTrees: Sendable, Equatable {
    package var left: [FileNode] = []
    package var right: [FileNode] = []
    /// Both sides merged into one tree, keyed by left-side paths.
    package var unified: [FileNode] = []
    /// The files git ignores on each side, and both together, when they are shown; empty otherwise.
    package var leftIgnored: [FileNode] = []
    package var rightIgnored: [FileNode] = []
    package var unifiedIgnored: [FileNode] = []
    ...
    package static func build(
        ...
        var unifiedNodes = FileNode.tree(from: Array(comparison.statuses.keys))
        ...
        if showsIgnoredFiles {
            ...
            trees.leftIgnored = arranged(FileNode.tree(from: Array(leftIgnored)), style: style)
            trees.rightIgnored = arranged(FileNode.tree(from: Array(rightIgnored)), style: style)
            trees.unifiedIgnored = arranged(FileNode.tree(from: Array(comparison.ignoredPaths)), style: style)
        }
        return trees
    }
    ...
    @discardableResult
    private mutating func aggregateStatus(of node: FileNode) -> PathStatus? {
